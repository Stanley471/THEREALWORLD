<x-dashboard-layout title="Complete Crypto Deposit">
    <style>
        ...
        .card { background: rgba(15,23,42,0.85); border: 1px solid rgba(148,163,184,0.12); border-radius: 1rem; padding: 1.5rem; }
        ...
        .value { color: #e2e8f0; font-size: 1.05rem; font-weight: 700; word-break: break-all; }
        ...
        .btn-primary { background: #34d399; color: #042016; }
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1>Complete Your {{ $pendingDeposit['currency'] }} Deposit</h1>
            <p>Send the exact amount to the address below. Your payment window expires in 30 minutes.</p>
        </div>
        ...
        <div class="card">
            <div class="details-grid">
                ...
                <div class="detail-box">
                    ...
                    <div class="value">{{ $selectedCrypto['label'] }}</div>
                </div>
                <div class="detail-box">
                    <div class="label">Network</div>
                    <div class="value">{{ $selectedCrypto['network'] }}</div>
                </div>
                <div class="detail-box">
                    <div class="label">Required Confirmations</div>
                    <div class="value">{{ $selectedCrypto['confirmations'] }}</div>
                </div>
            </div>

            <div class="detail-box" style="margin-bottom: 1rem;">
                <div class="label">Wallet Address</div>
                <div class="value">{{ $selectedCrypto['address'] }}</div>
            </div>

            <ol class="steps">
                <li>Copy the wallet address exactly as shown above.</li>
                <li>Send your {{ $pendingDeposit['currency'] }} payment from your wallet or exchange.</li>
                <li>After sending, click "I Have Sent the Payment" to submit your deposit for admin review.</li>
            </ol>

            <div class="actions">
                <form method="POST" action="{{ route('wallet.deposit.complete') }}">
                    @csrf
                    <button type="submit" class="btn btn-primary">I Have Sent the Payment</button>
                </form>
                <a href="{{ route('wallet.deposit') }}" class="btn btn-secondary">Start New Deposit</a>
            </div>
        </div>
    </div>
